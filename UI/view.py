...
class View:
    ...
    def load_interface(self):
        ...
        row1 = ft.Row([self.txtNumAlbumMin, self._btnCreateGraph], alignment=ft.MainAxisAlignment.CENTER)
        self._page.controls.append(row1)

        #row2
        self.ddArtist = ft.Dropdown(label="Artista", width = 250,on_change=self.controller.popola_dd())
        ...
        row2 = ft.Row([self.ddArtist, self.btnArtistsConnected], alignment=ft.MainAxisAlignment.CENTER)
        ...
        row3 = ft.Row([self.txtMinDuration, self.txtMaxArtists, self.btnSearchArtists], alignment=ft.MainAxisAlignment.CENTER)
        self._page.controls.append(row3)

        # txt_result
        self.txt_result = ft.ListView(expand=1, spacing=10, padding=20, auto_scroll=True)
        self._page.controls.append(ft.Text(f"il numero di ponti è:{self.controller.num_edges()}, il numero di nodi è:{self.controller.num_node()}"))
        self._page.update()
